...
if uploaded is None:
    st.info(
        ...
        "['unique_id','ds','y','y_hat','yhat_lower','yhat_upper','item_name','variation_name',"
        "'cluster_km8'(opt), 'model'(opt)]."
    )
    st.stop()

df0 = _coerce(_read_csv(uploaded)).dropna(subset=["ds"])
...
if search_txt:
    _txt = (
        (df_filter["item_name"].astype(str).str.lower().fillna(""))
        + " " +
        (df_filter["variation_name"].astype(str).str.lower().fillna(""))
    )
